...
## 9. Общая шапка, подвал и поиск (дополнение)

Добавлены общие компоненты для всего сайта и приложения:

- `assets/step3d-common.css` — единый стиль общей шапки, подвала, мобильной панели и поиска.
- `assets/step3d-common.js` — вставляет общую шапку/подвал, удаляет старые локальные header/footer/mobile bars, включает поиск по ключевым разделам.

Что реализовано:

- единая шапка на главной, страницах услуг, App, Viewer, Account, кейсах, статьях, галерее, служебных страницах;
- единый подвал с услугами, сценариями, разделами и контактами;
- поиск по сайту через кнопку «Поиск» и горячую клавишу `Ctrl/Cmd + K`;
- мобильная нижняя панель: Главная / Поиск / Услуги / Заявка;
- `service-worker.js` обновлён до `step3d-pwa-v5`, новые common CSS/JS добавлены в precache.

Проверка:

- common header/footer/search smoke: главная, app, viewer, services, account, gallery — OK;
- responsive overflow: 108 состояний, issues = 0;
- стандартные проверки Step3D — OK.

## 10. Минималистичный visual reset (дополнение)

После визуальной проверки предыдущий слой был признан слишком тяжёлым. Добавлен финальный минималистичный слой:

- `assets/step3d-minimal.css`

Что изменено:

- убран визуальный шум: лишние тени, градиенты, тяжёлые hover-эффекты;
- унифицированы карточки, кнопки, формы, таблицы и CTA в спокойном minimal industrial стиле;
- упрощён hero на мобильных;
- скрыта фиксированная нижняя мобильная панель, потому что она перекрывала CTA;
- сохранены общая шапка, подвал и поиск;
- `service-worker.js` обновлён до `step3d-pwa-v6`, чтобы не кэшировать старый тяжёлый интерфейс.

Проверка после reset:

- визуальный smoke 390px: критичных проблем первого экрана нет;
- responsive overflow: 108 состояний, issues = 0;
- стандартные проверки проекта — OK.
